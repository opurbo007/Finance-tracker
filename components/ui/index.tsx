    ...
    <div className="bottom-sheet-backdrop" onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className="bottom-sheet">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold font-display" style={{ color: 'var(--text)' }}>{title}</h2>
  ...
  return (
    ...
    >
      <div style={{
        ...
        <h3 style={{ fontSize: 17, fontWeight: 700, color: 'var(--text)', textAlign: 'center', margin: '0 0 8px', fontFamily: "'Syne', sans-serif" }}>
          {title}
        </h3>
        <p style={{ fontSize: 13, color: 'var(--text-3)', textAlign: 'center', margin: '0 0 24px', lineHeight: 1.5 }}>
          {message}
        </p>
...
// ── Metric Tile ───────────────────────────────────────────────────────────────
export function MetricTile({ label, value, valueColor, variant = 'default', className }: {
...
// ── Transaction Item ──────────────────────────────────────────────────────────
export function TransactionItem({ tx, onEdit, onDelete }: {
  tx: Transaction
  onEdit: () => void
  onDelete: () => void
}) {
    ...
    <div className="tx-row group">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs mt-0.5 truncate" style={{ color: 'var(--text-3)' }}>
          ...
          {' · '}{tx.paymentMethod}{' · '}{formatDate(tx.date)}
        </p>
      </div>
      <div className="flex items-center gap-1 flex-shrink-0">
        <div className="text-right">
          <span className="text-sm font-semibold font-display block"
...
function ActionBtn({ icon, onClick, color, label }: {
  icon: ReactNode; onClick: () => void; color: string; label: string
}) {
  return (
    <button
      onClick={onClick}
...
export function WealthCard({ account, onEdit, onDelete }: {
    ...
    <div className="type-toggle">
      {(['expense', 'income'] as const).map(t => (
        <button key={t} type="button" onClick={() => onChange(t)}
          className={cn('type-toggle-btn', value === t && (t === 'expense' ? 'active-expense' : 'active-income'))}>

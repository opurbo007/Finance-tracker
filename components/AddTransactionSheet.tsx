...
import {
  ALL_CATEGORIES, INCOME_CATEGORIES, PAYMENT_METHODS,
  type Transaction, type WealthAccount,
} from '@/types'
  ...
  return (
    <div>
      <p className="text-xs font-semibold mb-2" style={{ color: 'var(--text-3)' }}>
        Link to account <span style={{ color: 'var(--text-3)', fontWeight: 400 }}>(optional)</span>
      </p>
      ...
      <div className="flex flex-col gap-1.5 mb-3">
        ...
        <button
          type="button"
          onClick={() => { onSelect(''); onEffectChange('none') }}
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors"
          style={{
            background: selectedId === '' ? 'rgba(108,99,255,0.12)' : 'var(--surface)',
            border: `1px solid ${selectedId === '' ? 'var(--accent)' : 'var(--border)'}`,
          }}
        >
          <span className="text-base">🚫</span>
          <span className="text-sm font-medium" style={{ color: selectedId === '' ? '#a5a0ff' : 'var(--text-2)' }}>
            No account link
          </span>
        </button>
        ...
        {nonDebts.map(acc => (
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors"
            style={{
              background: selectedId === acc._id ? 'rgba(108,99,255,0.12)' : 'var(--surface)',
              border: `1px solid ${selectedId === acc._id ? 'var(--accent)' : 'var(--border)'}`,
            }}
          >
            <span className="text-lg leading-none">{acc.emoji}</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: selectedId === acc._id ? '#a5a0ff' : 'var(--text)' }}>
                {acc.name}
              </p>
            ...
            {selectedId === acc._id && (
              <span className="text-xs font-semibold" style={{ color: type === 'income' ? 'var(--emerald)' : 'var(--rose)' }}>
      ...
      {/* Effect override — only shown when account is selected */}
      {selectedId && (
        <div>
          ...
          <div className="flex gap-2">
            {(['add', 'deduct', 'none'] as const).map(e => (
              <button
                ...
                style={{
                  background: effect === e
                    ? e === 'add'    ? 'rgba(16,185,129,0.18)'
                    : e === 'deduct' ? 'rgba(244,63,94,0.18)'
                    :                  'rgba(255,255,255,0.06)'
                    : 'var(--surface)',
                  border: `1px solid ${effect === e
                    ? e === 'add'    ? 'rgba(16,185,129,0.4)'
                    : e === 'deduct' ? 'rgba(244,63,94,0.4)'
                    :                  'var(--border-2)'
                    : 'var(--border)'}`,
                  color: effect === e
                    ? e === 'add'    ? 'var(--emerald)'
              ...
              >
                {e === 'add' ? '↑ Credit' : e === 'deduct' ? '↓ Debit' : '○ No effect'}
              </button>
            ))}
          </div>
        </div>
...
export function AddTransactionSheet({ open, onClose, editTx }: Props) {
  ...
  const [wealthEffect,   setWealthEffect]   = useState<'add' | 'deduct' | 'none'>('none')
  ...
  useEffect(() => {
    if (editTx) {
      ...
      setCat('food'); setMethod('Cash'); setDate(todayISO())
      setLinkedWealthId(''); setWealthEffect('none')
    }
  }, [editTx, open])
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title={isEdit ? 'Edit Transaction' : 'Add Transaction'}>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input id="tx-amount" type="number" inputMode="decimal" placeholder="0.00"
              value={amount} onChange={e => setAmount(e.target.value)} className="input-field pl-9" />
          </div>
        </div>
        ...
        <div>
          ...
          <input id="tx-desc" type="text"
            placeholder={type === 'income' ? 'Source of income…' : 'What was this for?'}
            value={desc} onChange={e => setDesc(e.target.value)} className="input-field" />
        </div>
      ...
      {linkedWealthId && wealthEffect !== 'none' && (
        <div className="mt-4 px-4 py-3 rounded-xl text-sm"
          style={{
            background: wealthEffect === 'add' ? 'rgba(16,185,129,0.08)' : 'rgba(244,63,94,0.08)',
      ...
      <button type="button" onClick={handleSave} disabled={saving || !amount || !desc}

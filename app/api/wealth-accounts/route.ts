...
export async function GET(): Promise<NextResponse> {
  ...
  await connectDB();
  const docs = await WealthAccountModel.find({ userId })
    .sort({ createdAt: 1 })
    .lean();
  return NextResponse.json(
    docs.map((d) => toWealthAccount(d as Record<string, unknown>)),
  );
}
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  return NextResponse.json(
    toWealthAccount(doc.toObject() as unknown as Record<string, unknown>),
  );
}
